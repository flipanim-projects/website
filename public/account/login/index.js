(function () {
    ...
    let title = $('.title');
    setTimeout(function(){title.classList.remove('right');})
    ...
    let badRequest = {
        invalidCaptcha() { new Toast(['Invalid captcha', 'Please complete the captcha to confirm you\'re not an automated script']).init().show() },
        invalidCredentials() { new Toast(['Invalid username or password', '<a href="/account/forgotpassword" style="text-decoration: underline">Forgot your password?</a>', 10]).init().show() }
    }
    submit.onclick = () => {
        ...
        fetch('/api/v1/login', {
            ...
            .then(r => {
                ...
                if (r.message === "400 Bad Request: Invalid Captcha") return badRequest.invalidCaptcha();
                if (r.status === 302) {
                    login.classList.add('logged-in');
                    window.location.href = '/'
                } else if (r.message === 'Username or password is incorrect') return badRequest.invalidCredentials()
            })
    }
