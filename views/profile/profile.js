function FlipAnimProfile(user) {
    ...
    if (!query && user) window.location.href = '?user=' + (uid.value)
    ...
    function loadProfile(data) {
        ...
        html($('.profile-status'), `<span class="profile-status-type ${statuses[user.status.type]}">${statuses[user.status.type]}</span>${user.status.name ? ' | ' + user.status.name : ''}`)
